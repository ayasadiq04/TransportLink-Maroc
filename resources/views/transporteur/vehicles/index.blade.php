                    ...
                    @foreach($vehicles as $vehicle)
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col justify-between hover:shadow-md transition">
                            ...
                            <div class="pt-4 border-t border-gray-100 flex items-center justify-between">
                                ...
                                @if($vehicle->available)
                                    <form action="{{ route('transporteur.vehicles.destroy', $vehicle) }}"
                                          method="POST"
                                          onsubmit="return confirm('Confirmer la suppression de ce véhicule ?')"
                                          class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="inline-flex items-center gap-1 text-sm font-semibold text-rose-600 hover:text-rose-800 transition">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
