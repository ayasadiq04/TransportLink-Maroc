                        ...
                        <tbody class="divide-y divide-gray-100 bg-white">
                            @foreach($requests as $req)
                                <tr class="hover:bg-gray-50/80 transition">
                                    <td class="px-6 py-4">
                                        <a href="{{ route('admin.transport-requests.show', $req) }}" class="font-semibold text-indigo-600 hover:text-indigo-900">
                                            {{ $req->title }}
                                        </a>
                                        <div class="text-xs text-gray-500">{{ $req->goods_type ?? $req->cargo_type }} • {{ $req->weight ? $req->weight.'kg' : 'Poids N/A' }}</div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-xs text-gray-600">
                                        {{ $req->departure_city }} &rarr; {{ $req->destination_city ?? $req->arrival_city }}
                                    </td>
                                    <td class="px-6 py-4 text-xs font-semibold text-gray-900 whitespace-nowrap">
                                        {{ ($req->estimated_budget ?? $req->budget) ? number_format($req->estimated_budget ?? $req->budget, 2).' DH' : 'Non précisé' }}
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-right text-xs whitespace-nowrap">
                                        <a href="{{ route('admin.transport-requests.show', $req) }}" class="text-indigo-600 hover:text-indigo-900 font-semibold mr-3">
                                            Détails
                                        </a>
                                    </td>
                                </tr>
