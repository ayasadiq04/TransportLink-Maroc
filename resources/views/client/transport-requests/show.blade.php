                                            ...
                                            <div class="flex-shrink-0 text-right">
                                                ...
                                                @if($transportRequest->status === 'pending' && $offer->status === 'pending')
                                                    <div class="flex gap-2 mt-3 justify-end">
                                                        <button type="button"
                                                                x-data=""
                                                                x-on:click.prevent="$dispatch('open-modal', 'confirm-offer-accept-{{ $offer->id }}')"
                                                                class="px-3 py-1.5 bg-emerald-600 text-white text-xs font-semibold rounded-lg hover:bg-emerald-700 transition-colors">
                                                            ✓ Accepter
                                                        </button>
                                                        <button type="button"
                                                                x-data=""
                                                                x-on:click.prevent="$dispatch('open-modal', 'confirm-offer-reject-{{ $offer->id }}')"
                                                                class="px-3 py-1.5 bg-red-100 text-red-700 text-xs font-semibold rounded-lg hover:bg-red-200 transition-colors">
                                                            ✗ Rejeter
                                                        </button>

                                                        <x-confirm-modal
                                                            :name="'confirm-offer-accept-'.$offer->id"
                                                            title="Accepter cette offre"
                                                            message="Accepter cette offre ? Cela créera une mission et rejettera les autres offres."
                                                            :action="route('client.offers.accept', $offer)"
                                                            method="POST"
                                                            confirm-text="Accepter"
                                                            tone="warning"
                                                        />
                                                        <x-confirm-modal
                                                            :name="'confirm-offer-reject-'.$offer->id"
                                                            title="Rejeter cette offre"
                                                            message="Rejeter cette offre ?"
                                                            :action="route('client.offers.reject', $offer)"
                                                            method="POST"
                    ...
                    @if($transportRequest->status === 'pending')
                        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                            ...
                            <div class="space-y-2">
                                ...
                                <button type="button"
                                        x-data=""
                                        x-on:click.prevent="$dispatch('open-modal', 'confirm-request-delete')"
                                ...
                                <x-confirm-modal
                                    name="confirm-request-delete"
                                    title="Supprimer la demande"
                                    message="Supprimer définitivement cette demande ? Cette action est irréversible."
                                    :action="route('client.transport-requests.destroy', $transportRequest)"
                                />
                            </div>
                        </div>
                    @endif
